...
# How often the "typing…" chat action is re-sent, per CHAT, while any busy
# card in that chat is live (roadmap 8.24; per chat since 8.30). 0 — or any
# value <= 0 — disables the indicator outright.
#
...
# its own task (`animation._animate_typing`), ONE per chat, whose period is
# this value measured from the start of each send — NOT on the busy card's
# wake grid, which would round it up to the card's interval and let the
# status lapse.
#
# It IS paced by the per-chat budget, as the chat's LOWEST ornament (8.30).
# 8.24 had made it exempt — live probes on 2026-09-12 saw chat actions
# answer 200 inside a `retry_after=10` window that refused every edit —
# which proved typing is not blocked by an edit 429, not that it is free.
...
# 15 s — past Telegram's 5 s status, so an older turn's bubble flickers
# rather than staying lit. The goal is "flicker, never dark": at 4.5 s the
# bubble alone is 800 calls an hour, and on the vm3 replay it tripped the
# hourly shed (75 % of the ornament share) and went dark for 43 minutes at
# a time; with 9 s from 10 minutes it was still dark for 22.6, and with
# 15 s (240 an hour) for none. Tier 3 is its own constant so it can be
# retuned apart, and today equals tier 2. Off with the `card_age_decay`
# preference, like the card.
TYPING_AGE_TIER2_INTERVAL: float = 15.0
...
# The pinned "needs you" bar (8.31): one pinned message per scope chat,
# edited only when what it SHOWS changed (a session's state, a prompt, a
# flood regime), and then at most once per this many seconds per chat. A
# change inside the gap is not dropped: one trailing edit goes out when
# the gap ends, carrying the state as it is then. 30 s bounds the bar to
# 120 edits an hour in the worst case; in practice it edits on a handful
# of transitions an hour. (8.30's hook-driven dashboard, which it
# replaces, was 75 % of all calls in the QA replay before its debounce.)
PINNED_MIN_EDIT_GAP: float = 30.0
# A pinned message the user deleted is re-created (sent and pinned again,
# which adds a "pinned" service notice) at most this often per chat.
PINNED_RECREATE_MIN_INTERVAL: float = 3600.0
