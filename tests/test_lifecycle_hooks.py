...
Coverage targets (design.md success criteria not already tested):
  ...
  - bootstrap _HOOK_EVENTS: exactly 16 events including all four new ones
...
def test_bootstrap_hook_events_count_is_16(tmp_path, monkeypatch):
    """After bootstrap, settings.json must have exactly 16 hook events.

    Success criterion: the four lifecycle events took the total from 10 to 14,
    MessageDisplay — the source of the busy card's live commentary —
    brought it to 15, and PreModelSwitch (roadmap 8.35) brings it to 16.
    """
    from aipager import claude_bootstrap
    ...
    assert len(hook_events) == 16, (
        f"Expected 16 hook events, got {len(hook_events)}: {sorted(hook_events)}"
    )
