...
def test_every_chat_scoped_endpoint_is_metered_by_the_same_chat_bucket(
    ...
    ``sendChatAction`` is not in this list: it is metered by the same
    bucket again since 8.30, but as a SKIP-kind ornament it is refused
    rather than paced, so it has its own row below. It used to be listed here and the row still
...
def test_the_chat_action_endpoint_is_metered_by_the_chat_bucket(
    limiter, clock, run_async,
):
    """The sibling of the row above, AMENDED by 8.30 R1 (was
    ``test_the_chat_action_endpoint_is_not_metered_by_the_chat_bucket``).
    The endpoint is metered by the chat's bucket again, as its LOWEST
    ornament: with the burst drained it is REFUSED — never waiting, never
    sent — where 8.24 sent all four at once off-budget. 8.24's evidence
    (typing answered 200 inside an edit 429) proved typing is not blocked
    by an edit 429, not that it is free: on 2026-09-23 the uncounted
    bubble took a 429 of its own and the vm3 DM a 7-hour ban.

    Mutation: put ``sendChatAction`` back in ``_CHAT_BUDGET_EXEMPT`` and
    all four go out.
    """
    ...
    run_async(asyncio.wait_for(burst(), timeout=10))
    start = log[0][2]
    assert [round(t - start, 6) for _, _, t in log] == [0, 0, 0]
    chat = next(c for c in limiter.snapshot()["chats"]
                if c["chat_id"] == PRIVATE)
    assert chat["chat_actions"] == 0
