...
class _Wire:
    ...
    def methods(self) -> list[str]:
        """Every call but the "typing…" chat action, which runs on its own
        per-chat loop and is asserted on its own where it matters."""
...
def test_a_card_less_self_woken_turn_still_shows_typing(
    ...
    """R3: the typing bubble is not deferred with the card — it is the
    one sign in the chat list that the session is working — and it stops
    with the turn."""
    bot = _wire_bot(mk_bot, wire)
    sess = _busy_sess(bot)
...
# ── interplay with the long-run budget (8.30) and the pinned bar (8.31) ─────

def test_another_cards_stop_leaves_the_bubble_to_a_card_less_self_woken_turn(
    mk_bot, run_async, wire, gate,
):
    """8.30 made the bubble one loop per CHAT; 8.32 lets a self-woken turn
    light it before its card exists. When another session in the chat
    takes its card down, the loop is still this chat's: the card-less
    turn's armed deferral wants it. (Guard: `_stop_typing`'s
    ``still_wanted`` counting ``lazy_card_at``.)"""
    bot = _wire_bot(mk_bot, wire)
    cat = _busy_sess(bot)
    dog = bot.registry.get_or_create("claude-dog")
    dog.label = "dog"
    dog.scope_chat_id = CHAT
    dog.scope_kind = "dm"
    bot.registry.transition(dog.name, Status.BUSY)

    async def _scenario():
        await bot.notify(cat, "user_prompt_submit", {"self_woken": True})
        await _yield()
        loop = cat.typing_task
        assert loop is not None and not loop.done()
        dog.typing_task = loop           # dog's card had joined the loop
        bot._stop_animation(dog)         # ...and dog's turn ends
        await _yield()
        alive = not loop.done()
        bot._cancel_lazy_card(cat)
        bot._stop_animation(cat)
        await _yield()
        return alive

    assert run_async(_bounded(_scenario())) is True


def test_a_lazy_card_is_paced_on_the_turns_age_not_its_own(
    mk_bot, run_async, wire, gate,
):
    """The deferred card joins 8.30's age-decayed edit gate as a card of
    the turn it belongs to: its age is the turn's (the reset the deferral
    ran at the prompt), not the moment the card was finally sent."""
    ...
    async def _scenario():
        await bot.notify(sess, "user_prompt_submit", {"self_woken": True})
        await _yield()
        # The turn is eleven minutes old by the time it first does work
        # (the 30 s tier).
        sess.busy_started_at -= 660
        await bot.notify(sess, "tool_use", {
            "tool_name": "Bash", "tool_summary": "Bash: ls",
        })
        await _yield()
        assert sess.busy_msg_id and sess.busy_msg_id > 0
        now = time.monotonic()
        sess.last_tool_edit_at = now - 12
        due = bot._card_edit_due(sess, now, base_gap=STREAM_EDIT_INTERVAL)
        bot._stop_animation(sess)
        await _yield()
        return due

    assert run_async(_bounded(_scenario())) is False


def test_a_card_elsewhere_in_the_chat_reserves_the_card_less_turns_bubble(
    mk_bot, run_async, wire, gate,
):
    """8.30's tier-0 reservation: while a self-woken turn keeps the chat's
    bubble lit with no card of its own, another card in that chat plans
    its cadence with the bubble's share set aside."""
    bot = _wire_bot(mk_bot, wire)
    cat = _busy_sess(bot)
    dog = bot.registry.get_or_create("claude-dog")
    dog.label = "dog"
    ...
    async def _scenario():
        await bot.notify(cat, "user_prompt_submit", {"self_woken": True})
        await _yield()
        _interval, reserved = bot._card_pacing(dog, streaming=True)
        bot._cancel_lazy_card(cat)
        bot._stop_animation(cat)
        await _yield()
        return reserved
...
def test_the_pinned_bar_follows_a_turn_whose_card_was_never_sent(
    mk_bot, run_async, wire, gate,
):
    """8.31's bar is rendered from session state, not from the card: a
    card-less self-woken turn reads as working while it runs, and its end
    ...
    bot._maybe_update_bot_name = _spy

    async def _scenario():
        await bot.notify(sess, "user_prompt_submit", {"self_woken": True})
        await _yield()
        during = bot._render_pinned(CHAT)[0].splitlines()[0]
        before = len(refreshes)
        bot.registry.transition(sess.name, Status.IDLE)
        await bot.notify(sess, "idle_prompt", {"summary": ANSWER})
        await _drain_background()
        after = bot._render_pinned(CHAT)[0].splitlines()[0]
        return during, before, len(refreshes), after
