...
### Fixed
- **No more 👀 blinking into 👍.** A prompt sent to an idle session showed 👀 and then, a moment later, 👍. A reaction now waits about a second before it is sent, and if a newer one for the same message arrives in that time, only the newer one goes out (one call instead of two).
...
- **Claude Code commands get 👌 once they have run.** A command button
  ...
  say so later. Tapped while a turn runs (not `/model`, which is refused
  until the turn ends), it is 👀 until that turn ends —
...
- **A chat can no longer be banned for long-run volume that every
  ...
  against the real daemon code with both sessions streaming at once:
  under 800 calls in the busiest hour and about 1,900 in total, where the
  old code is modelled at ~11,500. The typing bubble is never dark and
  minimal mode is never entered.
...
- A photo, document or album captioned `/<label> …` now goes to that
  session with the prefix stripped, the way a text message does. It
  used to land on whichever session was last active with the caption
  kept verbatim, so Claude Code read `/<label>` as a slash command,
...
- A new prompt sent while a previous job's background agent was still
  ...
  only forgot the card. The superseded card is now settled in place first
  (final ✅ status line, Stop button removed, and the
...
- **Allow always could switch a session into auto mode** on Claude Code
  ...
  outside the session's working directory never offers Allow-always. Deny
  is unaffected (it still overshoots onto "No").
