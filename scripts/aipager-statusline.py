#!/usr/bin/env python3
"""StatusLine wrapper for Claude Code → aipager.
...
Reads statusLine JSON from stdin and does three things on every tick:
  1. Writes the raw JSON to /tmp/claude-status-<session>.json
     (hook_receiver.py reads this for fresh token counts on any hook event)
  ...
  3. Emits a short status line string to stdout (shown in Claude Code's terminal)

Stdlib-only — runs under /usr/bin/python3 or the project venv equally.
Failure modes are all swallowed: a missing daemon, bad JSON, or unwritable
/tmp must never break Claude Code's status bar.
"""
